...
if Dopplershift
    ...
    ylim([1e-3 1e0]);
    title('CDL-D, 3km/h@60GHz, $N_P = 5$','Interpreter','latex');
else
    semilogy(M_SNR_dB,mean(BER_FBMC_AWGN,2),            '-.','Color',[1 1 1]*0.25,'MarkerSize',MarkerSize,'Linewidth',Linewidth);
    legend('MIMO-ZF','MIMO-ML','MISO-Alamouti2x1 code 1 Path','MISO-Alamouti2x1 code 3 Paths','Theory, AWGN','Location','southwest');
    ylim([1e-3 1e0]);
    title('Lens Antenna Array, UPA, $N_P = 5$','Interpreter','latex');
end
